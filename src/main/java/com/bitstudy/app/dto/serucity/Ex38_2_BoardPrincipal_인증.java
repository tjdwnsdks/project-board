package com.bitstudy.app.dto.serucity;
...
public record Ex38_2_BoardPrincipal_인증(
    ...
    public static Ex38_2_BoardPrincipal_인증 of(String username, String password, String email, String nickname, String memo) {
        /* 권한정보를 컬렉션 데이터로 저장되기때문에 중복은 없다. 그래서 자료구조는 Set 이 되야한다.*/
        Set<Ex38_2_BoardPrincipal_인증.RoleType> roleTypes = Set.of(Ex38_2_BoardPrincipal_인증.RoleType.USER);

        return new Ex38_2_BoardPrincipal_인증(
                username,
                password,

                roleTypes.stream()
                        .map(Ex38_2_BoardPrincipal_인증.RoleType::getName)
    ...
    public static Ex38_2_BoardPrincipal_인증 from(UserAccountDto dto) {
        return Ex38_2_BoardPrincipal_인증.of(
                dto.userId(),
                dto.userPassword(),
                dto.email(),
                dto.nickname(),
    ...
    /* 다 했으면 config > SecurityConfig 로 돌아가기*/
}
