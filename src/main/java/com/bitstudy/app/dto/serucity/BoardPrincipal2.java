...
public record BoardPrincipal2(
    ...
    public static BoardPrincipal2 of(String username, String password, String email, String nickname, String memo) {
        /* 권한정보를 컬렉션 데이터로 저장되기때문에 중복은 없다. 그래서 자료구조는 Set 이 되야한다.*/
        Set<BoardPrincipal.RoleType> roleTypes = Set.of(BoardPrincipal.RoleType.USER);

        return new BoardPrincipal2(
                username,
                password,

                roleTypes.stream()
                        .map(BoardPrincipal.RoleType::getName)
                ...
                /** roleTypes 해설: 권환에 관련된 정보를 넣어줄거임.
                 ...
                 이렇게 하면 최초에 BoardPrincipal2 이 생성될때 인증정보를 만들게 되는데 new 가 돌면서 roleType이 만들어질거다.
                 */
                ,
                email,
                nickname,
